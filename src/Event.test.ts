...
describe('Event', () => {
  describe('#construct', () => {
    ...
    beforeAll(() => {
      setSDKInstance(AWS);
      mock('EventBridge', 'putEvents', async (): Promise<PutEventsResponse> => {
        return Promise.resolve({Entries: [{EventId: '123456'}]})
      })
    ...
    it('should fail with the use of validationMiddleware on wrong event', () => {
      ...
      const middyfiedHandler = middy(handler).use(myEvent.validationMiddleware())
        
      return expect(invoke(middyfiedHandler, {
    ...
    it('should succeed with the use of validationMiddleware on correct event', () => {
      const handler: Handler = (_event, _ctx, callback) => callback(null, 'returnValue');
      const middyfiedHandler = middy(handler).use(myEvent.validationMiddleware())
